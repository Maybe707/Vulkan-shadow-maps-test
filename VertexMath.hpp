...
template <typename T>
Matrix<T, 4> perspective(T r, T l, T t, T b, T n, T f) {
	mat4 perspective(static_cast<T>(0));

	perspective[0][0] = static_cast<T>(2) * n / (r - l);             // -1
	perspective[2][0] = (r + l) / (r - l);                           // 0 
	perspective[1][1] = static_cast<T>(2) * n / (t - b);             // -1
	perspective[2][1] = (t + b) / (t - b);                           // 0
	perspective[2][2] = -(f + n) / (f - n);                          // 0
	perspective[3][2] = -static_cast<T>(2) * f * n / (f - n);        // 1
	perspective[2][3] = -static_cast<T>(1);                          // -1

	return perspective;
}

template <typename T>
glm::mat4 perspective_glm(T r, T l, T t, T b, T n, T f) {
	glm::mat4 perspective(static_cast<T>(0));
